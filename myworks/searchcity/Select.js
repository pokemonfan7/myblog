...
// const city=["北京市","天津市","石家庄市","唐山市","秦皇岛市","邯郸市","邢台市","保定市","张家口市","承德市","衡水市","廊坊市","沧州市","太原市","大同市","阳泉市","长治市","晋城市","朔州市","晋中市","运城市","忻州市",
...
export default class Select extends Component {
    constructor(props){
        super(props);
        this.state={search:false,searchList:[],searchWord:"",city:[]};
        this.click=this.click.bind(this);
        this.listClick=this.listClick.bind(this);
        this.handleChange=this.handleChange.bind(this);
    }
    componentDidMount(){
        axios.get("https://5acc720e94587a0014eda82f.mockapi.io/select").then(
            response=>this.setState({
            city:response.data
        })
    ).catch(error=>console.log(error.message))
        axios.get("https://5acc760394587a0014eda833.mockapi.io/select1").then(
            response=>this.setState({
            city:[...this.state.city,...response.data]
        })
    ).catch(error=>console.log(error.message))
        axios.get("https://5acc782894587a0014eda837.mockapi.io/select2").then(
            response=>this.setState({
            city:[...this.state.city,...response.data]
        })
    ).catch(error=>console.log(error.message))
        axios.get("https://5acc78b994587a0014eda83b.mockapi.io/select3").then(
            response=>this.setState({
            city:[...this.state.city,...response.data]
        })
    ).catch(error=>console.log(error.message))
        axios.get("https://5acc797b94587a0014eda840.mockapi.io/select4").then(
            response=>this.setState({
            city:[...this.state.city,...response.data]
        })
    ).catch(error=>console.log(error.message))
    ...
    click(){
        if(this.a.className==="arrow1"){
            this.a.className="arrow2";
    ...
    handleChange(){
        ...
        // console.log(v);
        let searchList=this.state.city.filter(x=>x.indexOf(v)===0);
        this.setState({
            searchWord:v,
        ...
        return (
            <div>
                <div id="container" className="clearfix">
                    ...
                    <a className="arrow1" onClick={this.click} ref={a=>this.a=a} />
                </div>
                <div id="list" ref={div=>this.list=div}>
                    <ul  onClick={this.listClick}>
        {this.state.search?
                        this.state.searchList.map(
                            city=>
                        <li className="list-li" key={city}><strong>{this.state.searchWord}</strong>{city.slice(this.state.searchWord.length)}</li>
                         ):this.state.city.map(
                             city=>
                        <li className="list-li" key={city}>{city}</li>
                        )}
                    </ul>
                </div>
